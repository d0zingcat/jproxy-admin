<template>
  <div class="container">
    <Breadcrumb :items="['menu.sonarr', 'menu.sonarr.title']" />
    <a-card class="general-card" :title="t('menu.sonarr.title')">
      <a-row>
        <a-col :flex="1">
          <a-form
          ...
          >
            <a-row :gutter="16">
              <a-col :span="8">
                <a-form-item field="title" :label="$t('sonarr.title.title')">
                  <a-input
                    v-model="queryForm.title"
                    allow-clear
                    :placeholder="$t('sonarr.title.title.placeholder')"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item
                  ...
                  :label="$t('sonarr.title.tvdbId')"
                >
                  <a-input-number
                    v-model="queryForm.tvdbId"
                    :min="0"
                    allow-clear
                    :placeholder="$t('sonarr.title.tvdbId.placeholder')"
                  />
                </a-form-item>
              </a-col>
            </a-row>
        ...
        <a-col :flex="'86px'" style="text-align: right">
          <a-space :size="18">
            <a-button type="primary" :loading="searchLoading" @click="search">
              ...
              {{ $t('sonarr.title.search') }}
            </a-button>
            <a-button @click="reset">
              <template #icon>
                <icon-refresh />
              </template>
              {{ $t('sonarr.title.reset') }}
            </a-button>
          </a-space>
        </a-col>
      </a-row>
...
<script lang="ts" setup>
  ...
  const columns = computed<TableColumnData[]>(() => [
    ...
    {
      title: t('sonarr.title.mainTitle'),
      dataIndex: 'mainTitle',
      ellipsis: true,
      tooltip: true,
    },
    {
      title: t('sonarr.title.title'),
      dataIndex: 'title',
      ellipsis: true,
      tooltip: true,
    },
    {
      title: t('sonarr.title.cleanTitle'),
      dataIndex: 'cleanTitle',
      ellipsis: true,
      tooltip: true,
    },
    {
      title: t('sonarr.title.seasonNumber'),
      dataIndex: 'seasonNumber',
      slotName: 'seasonNumber',
      align: 'center',
      width: 100,
      ellipsis: true,
      tooltip: true,
    ...
    {
      title: t('sonarr.title.monitored'),
      dataIndex: 'monitored',
      slotName: 'monitored',
      align: 'center',
      width: 105,
    ...
    {
      title: t('sonarr.title.validStatus'),
  ...
  const remove = () => {
    if (!selectedKeys.value || selectedKeys.value.length === 0) {
      Message.warning(t('sonarr.title.select.warning'));
      return;
    }
    showLoading(removeLoading);
    removeSonarrTitle(selectedKeys.value as Array<number>)
      .then(() => {
        Message.success(t('sonarr.title.remove.success'));
        query({ ...pagination, ...queryForm.value });
      })
      .finally(() => {
  ...
  const sync = () => {
    ...
    syncSonarrTitle()
      .then(() => {
        Message.success(t('sonarr.title.sync.success'));
        search();
      })
      .finally(() => {
